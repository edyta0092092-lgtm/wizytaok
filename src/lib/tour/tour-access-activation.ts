 ...
 * sessionStorage = jednorazowy sygnał „właśnie aktywowano”; localStorage per firma = obsłużono welcome.
 */

export const ACCESS_ACTIVATION_SESSION_KEY = "pw_onboarding_access_just_activated"

const welcomeHandledPrefix = "pw_onboarding_welcome_handled_v1_"
...
export function welcomeHandledStorageKey(businessId: string): string {
  return `${welcomeHandledPrefix}${businessId}`
}
...
export function isWelcomeHandledForBusiness(businessId: string): boolean {
  const id = businessId.trim()
  if (!id || typeof window === "undefined") return false
  try {
    return window.localStorage.getItem(welcomeHandledStorageKey(id)) === "1"
  } catch {
    return false
  }
}

export function markWelcomeHandledForBusiness(businessId: string): void {
  const id = businessId.trim()
  if (!id || typeof window === "undefined") return
  try {
    window.localStorage.setItem(welcomeHandledStorageKey(id), "1")
  } catch {
    /* ignore */
  }
