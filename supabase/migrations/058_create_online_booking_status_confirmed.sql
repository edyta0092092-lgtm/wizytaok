-- =============================================================================
-- RĘCZNE WDROŻENIE — wklej całość w Supabase SQL Editor (nie przez CLI).
-- Cel: nowe wizyty online w RPC create_online_booking mają status confirmed.
-- Bazuje na: 031_create_online_booking_staff_services_compat.sql
-- Zmiany względem 031:
--   • status w INSERT: confirmed (nie booked)
--   • bez reminder_due_at / reminder_status (kolumny mogą nie istnieć na prod)
-- Przypomnienia: appointment_reminders tworzy istniejący trigger/mechanizm.
-- =============================================================================
...
-- =============================================================================
-- Weryfikacja po wykonaniu (uruchom osobno w SQL Editor):
--
-- 1) Definicja funkcji — brak reminder_due_at / reminder_status w INSERT:
-- select pg_get_functiondef(
--   'public.create_online_booking(text,uuid,text,text,text,date,time,text,uuid)'::regprocedure
-- );
--
-- 2) Ostatnie wizyty online:
-- select id, status, source, confirmation_token, created_at
