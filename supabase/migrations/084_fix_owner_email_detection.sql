-- Poprawka wykrywania e-maila właściciela (tylko user_id = owner_id, nie każdy wpis bez staff_member_id).
...
begin
  ...
  if v_owner_id is not null then
    select lower(trim(coalesce(bm.email, '')))
      into v_owner_email
    ...
    if v_owner_email <> '' and v_owner_email = v_em then
