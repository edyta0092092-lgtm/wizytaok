-- Poprawka: przypisanie zaproszenia do aktualnej osoby z zespołu, konflikt tylko gdy e-mail ma już aktywne konto panelu u innej osoby.
...
begin
  ...
  select bm.id, bm.staff_member_id
    into v_member_id, v_member_staff
  ...
  if v_member_id is not null then
    if v_member_staff is null then
      return jsonb_build_object('ok', false, 'code', 'owner_email');
    end if;

    if v_member_staff = p_staff_member_id then
