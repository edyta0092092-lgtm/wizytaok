-- Pierwsze przypomnienie (24h) tylko gdy termin wysyłki jest w przyszłości.
-- Wizyta zarezerwowana na <24h przed terminem dostaje wyłącznie drugie przypomnienie.

-- Anuluj błędnie zaplanowane pierwsze przypomnienia (okno 24h już minęło).
update public.appointment_reminders ar
set
  status = 'skipped',
  locked_at = null,
  last_error = 'first_reminder_window_passed',
  updated_at = now()
from public.bookings b
where ar.appointment_id = b.id
  and ar.reminder_kind = 'first'
  ...
  and ((b.appointment_date::timestamp + b.appointment_time) at time zone 'Europe/Warsaw') - now()
      < ((b.appointment_date::timestamp + b.appointment_time) at time zone 'Europe/Warsaw') - ar.scheduled_for - interval '30 minutes';
...
begin
  ...
  v_first_email_ok :=
       v_channel_has_email
   and v_has_email
   and v_first_scheduled > now() and v_appointment_ts > now();
